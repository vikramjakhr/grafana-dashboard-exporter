...
CONFIG_DIR = "/etc/gde"
CONFIG_DIR_D = "/etc/gde/gde.d"
LOGROTATE_DIR = "/etc/logrotate.d"

INIT_SCRIPT = "scripts/init.sh"
SYSTEMD_SCRIPT = "scripts/gde.service"
LOGROTATE_SCRIPT = "etc/logrotate.d/gde"
DEFAULT_CONFIG = "etc/gde.conf"
DEFAULT_WINDOWS_CONFIG = "etc/gde_windows.conf"
...
PACKAGE_LICENSE = "MIT"
PACKAGE_URL = "https://github.com/vikramjakhr/grafana-dashboard-exporter"
MAINTAINER = "vikram.jakhr@gmail.com"
VENDOR = "VikramJakhar"
DESCRIPTION = "Plugin-driven agent for backing up grafana dashboards"
...
def package_scripts(build_root, config_only=False, windows=False):
    ...
    if config_only or windows:
        ...
        if windows:
            shutil.copyfile(DEFAULT_WINDOWS_CONFIG, os.path.join(build_root, "gde.conf"))
        else:
            shutil.copyfile(DEFAULT_CONFIG, os.path.join(build_root, "gde.conf"))
        os.chmod(os.path.join(build_root, "gde.conf"), 0o644)
    else:
        ...
        shutil.copyfile(LOGROTATE_SCRIPT, os.path.join(build_root, LOGROTATE_DIR[1:], "gde"))
        os.chmod(os.path.join(build_root, LOGROTATE_DIR[1:], "gde"), 0o644)
        shutil.copyfile(DEFAULT_CONFIG, os.path.join(build_root, CONFIG_DIR[1:], "gde.conf"))
        os.chmod(os.path.join(build_root, CONFIG_DIR[1:], "gde.conf"), 0o644)
...
def run_tests(race, parallel, timeout, no_vet):
    # Currently a NOOP for gde
    return True

################
#### All gde-specific content above this line
################
                ...
                for package_type in supported_packages[platform]:
                    ...
                    if package_type in ['zip', 'tar']:
                        ...
                        fpm_command = "fpm {} --name {} -a {} -t {} --version {} --iteration {} -C {} -p {} ".format(
                            ...
                            current_location)
                        if package_type == "rpm":
                            fpm_command += "--directories /var/log/gde --directories /etc/gde --depends coreutils --depends shadow-utils --rpm-posttrans {}".format(POSTINST_SCRIPT)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='GDE build and packaging script.')
